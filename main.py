...
def validate_configuration() -> None:
    ...
    if missing:
        raise RuntimeError(
            "Configuração/arquivos ausentes: " + ", ".join(missing)
        )
...
def launch_child(name: str) -> subprocess.Popen:
    ...
    logger.info(
        "Iniciando %s | script=%s | token=%s | banco=%s | porta_local=%s",
        name,
        cfg["script"],
...
def monitor_child(name: str) -> None:
    while not stop_event.is_set():
        try:
            ...
            logger.error(
                "O processo %s encerrou com código %s. Reiniciando em 10 segundos.",
                name,
                exit_code,
            )
            stop_event.wait(10)
            ...
            logger.exception("Falha ao iniciar/monitorar o processo %s.", name)
            stop_event.wait(15)
